...
| RAG | 数据模型与 pgvector 已准备；Claims 预览只消费当前已加载证据，不冒充检索 | 混合检索、反证检索、Claim Context |
...
| PRD | 领域模型与引用规则已定义；可演示未持久化的 Claim / Opportunity 草稿与 revision 级证据边 | 持久化 Claim → Decision → cited PRD |
