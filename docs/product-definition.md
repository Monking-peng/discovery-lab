...
种子数据包含访谈、工单 CSV、NPS、行为指标、竞品网页和销售需求列表，并故意加入重复数据、分群冲突、陈旧资料、缺失字段和 Prompt Injection。
...
- 混合检索、支持证据和反面证据。
- Theme、Claim、Opportunity 和 Hypothesis。
- 至少两个 HITL 节点。
- Experiment Draft 和带引用 PRD。
- 一个带审批的真实外部写工具，优先 Linear。
- LangGraph 状态机、Checkpoint、失败恢复和 Run Inspector。
