...
代码按领域模块隔离。当前作品集运行态包含两个应用进程和两个基础设施容器：

1. `web`：Next.js 产品界面。
2. `api`：FastAPI 业务 API、权限、事务与控制平面。
3. `postgres`：PostgreSQL + pgvector，唯一业务事实源。
4. `redis`：为后续异步投递预留；当前核心演示不把权威状态放入 Redis。
...
```mermaid
flowchart LR
    USER["产品经理 / FDE"] --> WEB["Next.js 双语工作台"]
    WEB -->|"REST"| API["FastAPI Control Plane"]
    API --> DOMAIN["Application Services + Domain Rules"]
    DOMAIN --> PG["PostgreSQL + pgvector"]
    DOMAIN --> BLOB["Local Immutable Blob"]
    DOMAIN --> GRAPH["LangGraph Agent Harness"]
    GRAPH --> TOOLS["Server Tool Registry + Exact Approval"]
    MCP["Read-only FastMCP"] -->|"Public HTTP API only"| API
    EVAL["pytest + typed eval runner"] --> DOMAIN
```

解析与作品集 Agent Workflow 当前由 API 进程内的应用服务执行，以保证一键演示简单、结果确定且事务边界清晰。独立 Worker、Outbox/SSE 和可选 OTel/Langfuse 是生产扩展方向，不在当前 UI 中冒充已上线能力。
...
9. 人工把实验观察结果记录为 append-only Product Decision。
10. PRD Composer 只使用该 Decision、精确 Claim Revision 和 Context Manifest 中的 Evidence/Source Revision。
11. PRD 固定保持 `DRAFT`、`publishable=false`，直到未来实现最终审核与外部发布。
...
6. 当前唯一写工具只创建本地 Experiment Draft，并记录 `external_system_written=false`。
7. Linear/Jira 等外部发布未实现；未来接入时仍必须复用相同审批与幂等约束。
...
POST /v1/studies/{id}/retrievals
GET  /v1/context-manifests/{id}
POST /v1/studies/{id}/agent-runs
GET  /v1/agent-runs/{id}
POST /v1/tool-calls/{id}/approvals
GET  /v1/studies/{id}/product-artifacts
POST /v1/experiments/{id}/decisions
POST /v1/decisions/{id}/prds
GET  /v1/prds/{id}
GET  /v1/evaluations
GET  /v1/bad-cases
```

当前 Agent Run Inspector 通过 REST 读取 PostgreSQL 审计记录。SSE 脱敏事件流属于后续生产扩展，不是当前演示依赖。
